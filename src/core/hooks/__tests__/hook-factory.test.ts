...
describe("Hook System", () => {
	...
	const _WINDOWS_HOOK_TEST_TIMEOUT_MS = 15000
	const _WINDOWS_TEST_TIMEOUT_MS = 10000
	...
	describe("StdioHookRunner", () => {
		it("should execute workspace hook from its respective workspace root directory", async () => {
			if (process.platform === "win32") {
				// mocha this.timeout removed — vitest uses testTimeout config: (WINDOWS_HOOK_TEST_TIMEOUT_MS)
			}

			// Create a test hook script that outputs the current working directory
			const hookPath = path.join(tempDir, ".agents", "hooks", "PreToolUse")
  ...
  contextModification: "Received tool: " + input.preToolUse.toolName
	...
	describe("PostToolUse Hook", () => {
		it("should receive execution results", async () => {
			const hookPath = path.join(tempDir, ".agents", "hooks", "PostToolUse")
			const hookScript = `#!/usr/bin/env node
const input = JSON.parse(require('fs').readFileSync(0, 'utf-8'));
console.log(JSON.stringify({
  cancel: false,
...
console.log(JSON.stringify({
  ...
  contextModification: "GLOBAL: Context added"
			...
			const workspaceHookPath = path.join(tempDir, ".agents", "hooks", "PreToolUse")
