...
e2e("Chat input queue - a queued entry is delivered at the next turn end", async ({ helper, server, sidebar, userDataDir }) => {
	...
	expect(deliveredRequest).toContain("sent by the user while work was in progress")
	...
	await expect(deliveredInput).not.toContainText("sent by the user while work was in progress")
	await expect(deliveredInput).not.toContainText("用户在任务执行过程中发送")
