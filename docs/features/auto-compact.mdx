...
When your conversation approaches the model's context window limit, Dline automatically summarizes it to free up space and keep working.

## How It Works

Dline monitors token usage during your conversation. When you're getting close to the limit, it:
...
Previously, Dline would truncate older messages when hitting context limits, losing important context.
...
- Dline remembers everything it's done
- You can work on much larger projects without interruption

<Tip>
Auto Compact works especially well for long-running tasks. Structured task lists can help maintain progress across summarizations so Dline can stay on track across multiple context windows.
</Tip>
...
<Note>
With other models, Dline falls back to standard rule-based context truncation, even if Auto Compact is enabled.
</Note>
