...
Dline works with VSCode's multi-root workspaces, letting you manage multiple project folders or repositories in a single window. Whether you're working with a monorepo or separate Git repositories, Dline can read files, write code, and run commands across all of them.

<Warning>
Multi-root workspaces have two limitations:
- **Dline rules** only work in the primary workspace folder
...
Dline can complete tasks that span multiple projects or repositories:
...
Dline supports both patterns, as well as hybrid setups where some folders are Git repositories and others are not. The key difference: with multiple repositories, each folder has its own `.git` directory and Dline tracks them independently.
...
When you open separate Git repositories in one workspace, Dline treats each as an independent project with its own version control.

### What Dline Tracks Per Repository

For each workspace folder, Dline detects:
...
This means Dline understands that your frontend and backend might be at different commits, on different branches, or even use different version control systems.

<Note>
While Dline detects VCS information for all workspace folders, certain features only use the **primary workspace** (the first folder): [Dline rules](/customization/dline-rules), [skills](/customization/skills), and [Git-related features](/core-workflows/working-with-files) like `@git` mentions.
</Note>
...
Dline understands natural references to your workspaces:
...
- Dline needs to resolve ambiguity
...
Each service has its own repository. Dline can update the proto definitions and regenerate clients across all services.
...
The types repository defines interfaces used by both client and server. When you update a type, Dline can fix both consumers.
...
Mix of repositories and plain folders. Dline adapts to each folder's configuration.
...
[Dline rules](/customization/dline-rules) (`.agents/rules/` directory) only work in the **primary workspace** (the first folder in your workspace). Rules in other workspace folders are ignored.

**Workaround:** Place shared rules in the primary workspace, or use global rules (`~/Documents/dline/rules/`) which apply everywhere.

### Checkpoints

[Checkpoints](/core-workflows/checkpoints) are disabled in multi-root workspace mode. Dline displays a warning when this happens.
...
4. **Consider the primary workspace** for Dline rules placement
...
- Use [Plan mode](/core-workflows/plan-and-act) to let Dline understand structure first
- Add a `.agentignore` file to prune broad scans or restrict selected paths:
...
For permission attributes and examples, see the [.agentignore guide](/customization/agentignore).
