...
Every interaction with Dline happens within a task. Tasks are self-contained work sessions that capture your entire conversation, code changes, command executions, and decisions.

## What are Tasks?

A task begins when you submit a prompt to Dline. Your prompt defines the goal, and Dline works toward it through conversation, code changes, and tool use. The quality of your initial prompt directly affects how well Dline performs - clear, specific prompts lead to better results.
...
<Tip>
Clear goals, relevant context, and explicit acceptance criteria help Dline produce more reliable results.
</Tip>

## Scoping Your Tasks

Each task carries its own context: the conversation history, decisions made, and understanding built up over the session. How you scope your tasks directly affects how well Dline can help you.
...
Every AI model has a context window - a limit on how much information it can process at once. Think of it as Dline's working memory for the current task.

As you work, the context window fills up with:
- Your prompts and Dline's responses
- File contents Dline reads or edits
- Command outputs and tool results
- System instructions that guide Dline's behavior (including [Dline Rules](/customization/dline-rules))

When the context window approaches its limit, Dline automatically compresses older parts of the conversation to make room. This means very long tasks may lose some earlier details, though Dline preserves the most important context.
...
If broad file discovery is pulling in irrelevant paths, add a [`.agentignore`](/customization/agentignore) file to limit scanning or restrict access to selected files and directories.
...
Knowing when to start fresh versus continue can feel unclear at first. As you work with Dline more, you'll develop an intuition for it. Use this table as a starting point:

| Scenario | Action | Why |
|----------|--------|-----|
| Switching to a different feature | **New task** | Clean context, focused responses |
| Building on work Dline just completed | **Continue** | Shared understanding preserved |
| Dline keeps going off-track | **New task** | Fighting context wastes time |
...
| Refining Dline's last output | **Continue** | Momentum and decisions preserved |

To start a new task, click the **+** button in the Dline sidebar or use the `/newtask` command. Your file changes are preserved through [checkpoints](/core-workflows/checkpoints), and you can reference previous tasks from history anytime.

## Understanding Task Costs

Every cloud-based AI model charges for usage based on tokens, the units of text the model processes. Dline tracks these costs automatically and displays them in the task header so you can monitor spending as you work.

### How Costs Are Calculated

When you interact with Dline, the model processes:
...
Cloud providers charge per million tokens, with output tokens typically costing more than input. Some providers also support **prompt caching**, which reduces costs when the same context (like your Dline rules or large files) appears in multiple requests. Dline automatically tracks cache savings when available.
...
|--------------|---------------|
| **Dline Provider** | Pay-per-use with credits you purchase |
...
Not ready to pay? Dline offers several free paths:

- **Free models**: Search "free" in the model selector when using the Dline provider. These models display a **FREE** tag and work well for learning and experimentation.
...
For most users, starting with free cloud models and moving to paid options as needed provides the best balance of cost, speed, and capability. Check [Selecting Your Model](/getting-started/authorizing-with-dline) for guidance on choosing the right option for your workflow.
...
Click the **History** button in the Dline sidebar (clock icon at the top-right) to open the history view. You'll see all your past tasks with their initial prompt, timestamp, and token usage. Each task card expands to show a preview of the conversation.

### Searching Tasks

Use the search bar at the top of the history view to find specific tasks. The fuzzy search looks across everything: your prompts, Dline's responses, code snippets, and file names.
...
Dline can resume interrupted tasks with full context:

1. Open the task from history
2. Dline loads the complete conversation
...
This works across sessions. Even if you close the editor and return days later, Dline can pick up where you left off.
