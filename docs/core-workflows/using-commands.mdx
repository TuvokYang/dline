...
Dline provides slash commands in chat that help you manage your conversation and plan complex implementations.
...
| `/newrule` | Create a rule file to teach Dline your preferences |
...
`/newrule` creates a rule file that teaches Dline your preferences. Dline will guide you through communication style, coding standards, project context, and reusable practices. The rule is saved under `.agents/rules/` and automatically loaded for future conversations.

Use `/newrule` when you find yourself repeating the same instructions across tasks. For more about rules, see [Dline Rules](/customization/dline-rules).

### /deep-planning

Transform Dline into a meticulous architect who investigates your codebase, asks clarifying questions, and creates a comprehensive implementation plan before writing any code. Deep planning follows a four-step process:

1. **Silent Investigation** - Dline explores your codebase structure and patterns
...
`/reportbug` collects diagnostic information and helps you report issues with Dline. It gathers relevant context like your configuration, recent errors, and system details to make bug reports more useful for the development team.
...
- Select a skill command (for example, `/skills:aws-deploy`).
- Dline injects that skill's `SKILL.md` instructions into the current task.
...
For setup and management details, see [Skills](/customization/skills).
