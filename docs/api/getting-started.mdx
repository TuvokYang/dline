...
description: "Create an API key and make your first request to the Dline API in under a minute."
---
...
- A Dline account at [app.dline.bot](https://app.dline.bot)
- `curl` or any HTTP client (Python, Node.js, etc.)

## Create an API Key

<Steps>
  <Step title="Sign in to app.dline.bot">
    Go to [app.dline.bot](https://app.dline.bot) and sign in with your account.
  </Step>
...
| `402 Payment Required` | Your account has insufficient credits. Add credits at [app.dline.bot](https://app.dline.bot) |
| Empty response | Make sure `messages` is a non-empty array with at least one user message |
| Connection timeout | Verify your network can reach `api.dline.bot`. Check proxy settings if on a corporate network |
...
<CardGroup cols={2}>
  ...
  <Card title="Code Examples" icon="code" href="/api/sdk-examples">
    Use the API from Python, Node.js, or curl.
  </Card>
</CardGroup>
