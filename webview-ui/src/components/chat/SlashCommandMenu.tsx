		...
		return (
			<>
				...
				{commands.map((command, index) => {
					...
					const typeLabel =
						command.section === "default"
							? "Command"
							: command.section === "workflow"
								? "Workflow"
								: command.section === "skill"
									...
									: command.section === "mcp"
										? "MCP"
										: ""
					// Color-coded badge per section type
					const badgeClass =
						command.section === "default"
							? "bg-(--vscode-textLink-foreground)/15"
							: command.section === "workflow"
								? "bg-(--vscode-terminal-ansiGreen)/15"
								: command.section === "skill"
									? "bg-(--vscode-terminal-ansiYellow)/15"
									: command.section === "mcp"
										? "bg-(--vscode-terminal-ansiMagenta)/15"
										: ""
					const badgeColorVar =
						command.section === "default"
					...
					return (
						<div
							...
							className={`slash-command-menu-item py-2 px-3 cursor-pointer flex flex-row items-center border-b border-(--vscode-editorGroup-border) ${
							...
							{typeLabel && (
								<span
									className={`text-[0.8em] font-bold shrink-0 ml-2 px-1.5 py-0.5 rounded-full leading-normal ${badgeClass}`}
									style={
										itemIndex === selectedIndex
											? { backgroundColor: badgeColorVar, color: "#fff" }
											: { color: badgeColorVar }
									}>
									{typeLabel}
								</span>
			...
			<div
				...
				{filteredCommands.length > 0 ? (
					<>
						...
						{renderCommandSection(workflowCommands, "Workflows", defaultCommands.length, true)}
